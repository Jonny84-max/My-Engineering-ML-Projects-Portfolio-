...
def run():
    ...
    # ---------------- SAFETY + MAINTENANCE ----------------
    st.write(check_operational_safety(vessel_speed, roughness, days_since_clean))
    ...
    speed_after = [speed_loss_due_to_fouling(r, vessel_speed) for r in roughness_range]

    df_speed = pd.DataFrame({
        "Hull Roughness (mm)": roughness_range,
        "Speed After Fouling (kn)": speed_after
    })

    chart = alt.Chart(df_speed).mark_line(point=True).encode(
        x="Hull Roughness (mm)",
        y="Speed After Fouling (kn)"
    )
    ...
    fuel_vals = [
        fuel_consumption_tph(
            power_required(resistance_increase(r, vessel_speed), vessel_speed) / 1000
        )
